...
def init_context_parallel_mesh(
    device=None, *, mesh=None, max_batch_dim_size=None, max_ulysses_dim_size=None, max_ring_dim_size=None
):
    if mesh is not None:
        return mesh

    assert device is not None, "device must be provided if mesh is not provided"
    ...
    return dist.init_device_mesh(device, mesh_shape, mesh_dim_names=("batch", "ulysses", "ring"))
